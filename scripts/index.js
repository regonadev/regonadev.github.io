...
const discordId = "195250911323488257";
...
const parseStatus = data => {
  return {
    text: data.discord_status.replace('online', 'Online').replace('idle', 'Idle').replace('dnd', 'Do Not Disturb').replace('offline', 'Invisible'),
...
const parseActivity = (data, activityData) => {
  return activityData !== undefined ? `Listening "${data.spotify.song}" <a class="spotify-time">${activityData.current} ─ ${activityData.end}</a>` : 'Not listening right now.';
};

const reloadUser = async (data) => {
  clearInterval(currentInterval);
  $('div-img.avatar').css('background-image', `url(https://cdn.discordapp.com/attachments/1080540579894677576/1089484700306898964/1679823732065.jpg?size=512)`).removeAttr('loading');
  ...
  <a class="discord" href="https://discord.gg/927" target="_blank">Discord</a>`);
  ...
  ${data.spotify.artist.replace(/;/g, ',')}'s song`);
    activityTimer(data);
  } else {
...
function showGithubRepos() {
  const getGitHubData = async () => {
    const response = await fetch(properties.github_api.baseURL + '/users/regonadev/repos');
  ...
  const socials = [
    {
      ...
      href: 'https://instagram.com/regonae',
      tippy: 'Instagram'
    },
    {
      svg: `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" fill="currentColor" class="bi bi-spotify" viewBox="0 0 16 16" id="IconChangeColor"> <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" id="mainIconPathAttribute" fill="#ffffff"></path> </svg>`,
      href: 'https://open.spotify.com/artist/5qFm0RtIjVpGVSw6YgaQA2',
      tippy: 'Spotify'
    },
    {
      ...
      href: 'mailto:godregona@gmail.com',
      tippy: 'E-Mail'
    },
    {
    ...
    },
    {
      ...
      href: 'https://github.com/regonadev',
      tippy: 'GitHub'
    }
  ];
